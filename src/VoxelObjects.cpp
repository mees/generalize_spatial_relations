...
VoxelObjects::VoxelObjects(float width, float height, float depth, float resolution ,string filename1, string filename2) :  width(width),height(height), depth(depth), resolution(resolution), p1(filename1), p2(filename2), generator(rd()), distribution(0.5){
...
bool VoxelObjects::voxelize(octomap::ColorOcTree *my_colorOctree, const pcl::PointCloud<pcl::PointXYZ>::Ptr &cloud_in_a, const pcl::PointCloud<pcl::PointXYZ>::Ptr &cloud_in_b){
   ...
   if (!isnan (it->x) && !isnan (it->y) && !isnan (it->z)) {
      octomap::ColorOcTreeNode* result = my_colorOctree->search((*it).x, (*it).y, (*it).z);
       if(result!=NULL){ //if are not unknown
        ...
        if(!my_colorOctree->isNodeOccupied(result) || color!=color_obj1){ //check it's free or it's the same object
          my_colorOctree->updateNode((*it).x, (*it).y, (*it).z, true, false);
