...
Features::Features(float res, octomap::ColorOcTreeNode::Color colorObj1, octomap::ColorOcTreeNode::Color colorObj2) : generator(rd()), distribution(0.5) {
  resolution = res;
...
EigenVector Features::computeFeatureDescriptor(const pcl::PointCloud<pcl::PointXYZ>::Ptr &cloud_in_1, const pcl::PointCloud<pcl::PointXYZ>::Ptr &cloud_in_2)  {
  ...
  feature_vector << angle_1_vector, angle_2_vector,
                 distance_vector;
...
double Features::jensen_shannon_div(const EigenVector &p_src, const EigenVector &p_tgt)
{
  EigenVector masked_src(p_src.unaryExpr([](float v) -> float{ return v <= 0 ?  0.0000001 : v ; }));
  EigenVector masked_tgt(p_tgt.unaryExpr([](float v) -> float{ return v <= 0 ?  0.0000001 : v ; }));
  EigenVector term_a = masked_src.segment<18>(0).array() * (2 * masked_src.segment<18>(0).array() / (masked_src.segment<18>(0).array() + masked_tgt.segment<18>(0).array())).array().log();
  EigenVector term_b = masked_tgt.segment<18>(0).array() * (2 * masked_tgt.segment<18>(0).array() / (masked_src.segment<18>(0).array() + masked_tgt.segment<18>(0).array())).array().log();
  double firstHist = 0.5 * (term_a + term_b).array().sum() / log(2);
  term_a = masked_src.segment<18>(18).array() * (2 * masked_src.segment<18>(18).array() / (masked_src.segment<18>(18).array() + masked_tgt.segment<18>(18).array())).array().log();
  term_b = masked_tgt.segment<18>(18).array() * (2 * masked_tgt.segment<18>(18).array() / (masked_src.segment<18>(18).array() + masked_tgt.segment<18>(18).array())).array().log();
  double secondHist = 0.5 * (term_a + term_b).array().sum() / log(2);
  term_a = masked_src.segment<3>(36).array() * (2 * masked_src.segment<3>(36).array() / (masked_src.segment<3>(36).array() + masked_tgt.segment<3>(36).array())).array().log();
  term_b = masked_tgt.segment<3>(36).array() * (2 * masked_tgt.segment<3>(36).array() / (masked_src.segment<3>(36).array() + masked_tgt.segment<3>(36).array())).array().log();
...
double Features::kl_div(const EigenVector &p_src, const EigenVector &p_tgt)
{
  ...
  EigenVector masked_src(p_src.unaryExpr([](float v)-> float { return v <= 0 ?  0.0000001 : v ; }));
  EigenVector masked_tgt(p_tgt.unaryExpr([](float v) -> float{ return v <= 0 ?  0.0000001 : v ; }));
  double firstHist =  (masked_src.segment<18>(0).array() * ((masked_src.segment<18>(0).array() / masked_tgt.segment<18>(0).array()).array().log())).sum();
  double secondHist = (masked_src.segment<18>(18).array() * ((masked_src.segment<18>(18).array() / masked_tgt.segment<18>(18).array()).array().log())).sum();
  double thirdHist = (masked_src.segment<3>(36).array() * ((masked_src.segment<3>(36).array() / masked_tgt.segment<3>(36).array()).array().log())).sum();
...
double Features::bhattacharyya_dist(const EigenVector &p_src, const EigenVector &p_tgt)
{
  ...
  EigenVector masked_src(p_src.unaryExpr([](float v)-> float { return v <= 0 ?  0.0000001 : v ; }));
  EigenVector masked_tgt(p_tgt.unaryExpr([](float v) -> float{ return v <= 0 ?  0.0000001 : v ; }));
  double firstHist = -1 * log( (masked_src.segment<18>(0).array() * masked_tgt.segment<18>(0).array()).array().sqrt().sum() );
  double secondHist = -1 * log( (masked_src.segment<18>(18).array() * masked_tgt.segment<18>(18).array()).array().sqrt().sum() );
  double thirdHist = -1 * log( (masked_src.segment<3>(36).array() * masked_tgt.segment<3>(36).array()).array().sqrt().sum() );
  return firstHist + secondHist + thirdHist;
}
...
void Features::downsamplePointCloud(const pcl::PointCloud<pcl::PointXYZ>::Ptr &cloud_in,  pcl::PointCloud<pcl::PointXYZ>::Ptr &cloud_out, float resolution) {
  pcl::VoxelGrid<pcl::PointXYZ> sor;
  sor.setInputCloud (cloud_in);
  sor.setLeafSize (resolution, resolution, resolution);
  sor.filter (*cloud_out);

  std::cout << "PointCloud after filtering: " << cloud_out->width * cloud_out->height
